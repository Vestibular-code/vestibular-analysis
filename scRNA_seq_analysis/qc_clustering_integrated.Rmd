...
# Integrated data
v0402<-readRDS(paste0("/path_to_your_result/", "final_qc_annnotated.rds"))
v0402<-readRDS(paste0("/path_to_your_result/", "final_qc_annnotated.rds"))
v0618<-readRDS(paste0("/path_to_your_result/", "final_qc_annnotated.rds"))
v0909<-readRDS(paste0("/path_to_your_result/", "final_qc_annnotated.rds"))
...
VG.list = c(v0402,v0402,v0618,v0909)
...
combined <- IntegrateData(anchorset = VG.anchors, normalization.method = "SCT")
...
Idents(combined) = combined$integrated_snn_res.3
combined = subset(combined, idents = c(combined_result), invert = TRUE)
combined = subset(combined, subset = scDblFinder.class == "singlet")
combined = subset(combined, subset = nFeature_RNA > 200)

combined <- RunPCA(combined, verbose = FALSE)
combined <- RunUMAP(combined, reduction = "pca", dims = 1:50, verbose = FALSE)
DimPlot(combined, reduction = "umap")

Annotation <- function(label, gene_list){
  anno.scaled = ScaleData(combined, features = gene_list, vars.to.regress = c("percent.mt"), verbose = FALSE)
  # raw counts
  p = VlnPlot(anno.scaled, features = gene_list, layer = "counts", pt.size = 0, stack = T, flip = T)
  # log Normalized Count
  p = VlnPlot(combined, features = gene_list, layer = "data", pt.size = 0, stack = T, flip = T)
  # feature plot
...
new.cluster.ids <- c("schwann","endothelial cells","neuron","satelite glia","fibroblast","neuron","neuron","macrophages","satelite glia","neuron","neuron","SMCs","oligo","fibroblast")

names(new.cluster.ids) <- levels(combined)
combined <- RenameIdents(combined, new.cluster.ids)
combined$final_cluster = Idents(combined)

DimPlot(combined, reduction = "umap", label = TRUE)

saveRDS(combined, paste0("/path_to_your_result/", "clustered.rds"))
