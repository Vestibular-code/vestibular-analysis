...
def plot_summary_scatter_all(summary_folder, marker_names, bin_num):
    ...
    for sam_name in unique_sam_names:
        for i in range(1):
            ...
            positive_idx = summary_data['hasPositive'] == 1 if 'hasPositive' in summary_data.columns else np.ones(len(summary_data), dtype=bool)
            pos_x = standardized_x[positive_idx].values
            pos_y = standardized_y[positive_idx].values
